...
master_label = 'T3_NanoAodPostProcessingUL_DAS_{tstamp}'.format(tstamp=timestamp_tag)


input_path   = "/store/user/"
output_path  = "/store/user/$USER/%s/%s/%s" % (production_tag,UL_YEAR,out_ver)
workdir_path = "/tmpscratch/users/$USER/FullProduction/DAS/%s/%s/%s" % (production_tag,UL_YEAR,out_ver)
plotdir_path = "~/www/lobster/FullProduction/DAS/%s/%s/%s" % (production_tag,UL_YEAR,out_ver)
...
for key, value in SAMPLES.items():
    if value[5]=='DAS':
        ...
        Analysis = Workflow(
            ...
            outputs=['tree.root'],
            dataset=cmssw.Dataset(dataset=value[0][0], file_based=True),
